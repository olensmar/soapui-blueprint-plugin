...
class BlueprintImporter {
    ...
    public RestService importBlueprint(String url) {

        def service = createRestService(null)

        return service
    }

    private RestService createRestService(def blueprint) {
        RestService restService = project.addNewInterface(blueprint.title, RestServiceFactory.REST_TYPE)
